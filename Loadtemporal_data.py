...
class VIPL_train(Dataset):
    ...
    def get_single_video_x(self, video_path, start_frame):
        # Path to the folder containing the images on your desktop
        image_folder_path = os.path.join(self.root_dir, video_path)
        #image_folder_path = os.path.join(self.root_dir, video_path)
        ...
        for i in range(clip_frames):
            ...
            if tmp_image is None:  # It seems some frames missing
                #tmp_image = cv2.imread(os.path.join(self.root_dir, r'C:/Users/AMK/Desktop/p10/v1/source1/image_00737.png'))
                #video_path = cv2.imread(self.root_dir, 'p10', 'v1', 'source1', 'image_00031.png')
                image_path = os.path.join(self.root_dir, 'p10', 'v1', 'source1', 'image_00031.png')
                tmp_image = cv2.imread(image_path)
        ...
        if clip_average_HR > 88:  # halve
            ...
            for tt in range(clip_frames):
                if tt % 2 == 0:
                    ...
                    #added
                    if image_id > 803:  # Adjust 803 if your last frame has a different index
                        break
                        #close added
                    s = "%05d" % image_id
                    image_name = 'image_' + s + '.png'
                    ...
                    tmp_image = cv2.imread(image_path)
                    if tmp_image is None:  # It seems some frames missing
                        fallback_path = os.path.join(self.root_dir, 'p10', 'v1', 'source1', 'image_00031.png')
                        tmp_image = cv2.imread(fallback_path)
                    ...
                    if tmp_image1 is None:  # It seems some frames missing
                        fallback_path = os.path.join(self.root_dir, 'p10', 'v1', 'source1', 'image_00041.png')
                        tmp_image = cv2.imread(fallback_path)
