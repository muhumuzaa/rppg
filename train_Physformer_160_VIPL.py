...
from Loadtemporal_data import VIPL_train, Normaliztion, ToTensor, RandomHorizontalFlip #removed #VIPL_test as model doesn't use it
...
def train_test():
    ...
    #adjusting root to the below coz colab couldn't find the train_txt file
    #VIPL_root_list = args.input_data 
    VIPL_train_list = 'VIPL_fold1_train.txt'
    #VIPL_test_list = 'VIPL_fold1_test.txt'

    train_txt_path = os.path.join(VIPL_root_list, VIPL_train_list)
    if not os.path.exists(train_txt_path):
      print(f"Train file not found at: {train_txt_path}")
    else:
      print(f"Train file found at: {train_txt_path}")


    print("Looking for CSV file at:", os.path.join(VIPL_root_list, VIPL_train_list))  
    ...
    for epoch in range(args.epochs):
        ...
        VIPL_trainDL = VIPL_train(train_txt_path, VIPL_root_list, transform=transforms.Compose([Normaliztion(),RandomHorizontalFlip(), ToTensor()]))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_data', type=str, default="/content/drive/MyDrive/phys/") 
